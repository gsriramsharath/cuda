...
#ifdef VERSION1
#define CUBINFILE		"dgemm_kernels_sass_v1.cubin"
#endif

#ifdef VERSION2
#define CUBINFILE		"dgemm_kernels_sass_v2.cubin"
#endif

#ifdef VERSION3
#define CUBINFILE		"dgemm_kernels_sass_v3.cubin"
#endif

#ifdef VERSION4
#define CUBINFILE		"dgemm_kernels_sass_v4.cubin"
#endif

#ifdef VERSIONZerobeta
#define CUBINFILE		"dgemm_kernels_sass_vZerobeta.cubin"
#endif
...
static int fermi_dgemm_init(void)
{
  ...
  if((res = cuModuleLoad(&mod_help, "helper_nvcc.cubin")) != CUDA_SUCCESS) return(ERRCODE(res));
  if((res = cuModuleLoad(&mod_kern, CUBINFILE)) != CUDA_SUCCESS) return(ERRCODE(res));
...
int fermiCreateStreams(int count)
{
  ...
  for(i = 1; i <= count; i++) {
    ...
    if(!use_native_nn_kernel) {
      if((res = cuMemAlloc(&buffers[i], MAX_TRANSPOSE)) != CUDA_SUCCESS) return(ERRCODE(res));
    }
  }
  num_streams = count;
